---
title: "TFM; Meteorología y Cambios climáticos: Efectos en un Glaciar de los Alpes"
autor: "Bryan Jimenez"
format: html
editor: visual
---
...
El presente estudio tiene como objetivo analizar el impacto del cambio climático en el glaciar Aletsch, ubicado en los Alpes suizos. Para ello, se construirán modelos estadísticos que expliquen el impacto de cada una de estas variables sobre el balance de masa, identificando aquellas con mayor incidencia en el comportamiento del glaciar.

De manera más específica, se evaluará la tendencia del balance de masa en relación con los efectos del cambio climático, utilizando análisis de datos y comparación de modelos estadísticos. Este enfoque busca mejorar la comprensión de cómo los cambios en las condiciones climáticas afectan la dinámica del glaciar Aletsch, aportando evidencia relevante para la discusión sobre los impactos del cambio climático en los glaciares alpinos.
...
datos_combinados[!complete.cases(datos_combinados), ] |>
  kable() |>
...
En los datos analizados, se detectaron 108 valores faltantes de precipitación, concentrados principalmente entre 2017 y 2019. En ciertos períodos, como enero de 2019, diciembre de 2018 y enero de 2017, hay ausencias de datos que abarcan casi un mes completo.

La precipitación no muestra un patrón diario definido, pero al examinar su comportamiento mensual, se observa que algunos meses registran lluvias notablemente más intensas que otros. Dado que existen lapsos prolongados sin registros (como meses casi incompletos), no es viable aplicar métodos de interpolación directa para imputar los datos faltantes. Al analizar la distribución histórica, se identificó que diciembre, enero, marzo y abril suelen ser los meses con mayores volúmenes de precipitación. Abril resulta especialmente útil para este análisis, ya que, a diferencia de los otros meses, no presenta valores faltantes en ningún año.

Para imputar los datos faltantes, se procedió de la siguiente manera: 1. Se calcularon la media y la desviación estándar de la precipitación en abril para cada año. 2. Los meses con datos incompletos (como enero o diciembre) se completaron generando valores aleatorios basados en la distribución estadística de abril del mismo año.

Este método asegura que se mantengan tanto la media anual como la variabilidad natural de los datos, evitando introducir sesgos artificiales y respetando los patrones estacionales observados.
...
El modelo con todas las variables introducidas tiene un R2 ajustado=0.5628, lo que quiere decir que es capaz de explicar el 56.28% de la variabilidad observada en el balance de masas y un p-value significativo, por lo que se puede aceptar que el modelo no es por azar.
...
-   Hipótesis alternativa (H₁): Los errores presentan heterocedasticidad (varianza no constante).
...
Test de Box-Ljung El test de Box-Ljung es una prueba estadística utilizada para evaluar si los residuos de un modelo de series temporales son independientes (es decir, si no hay autocorrelación). (Box, 1970)
...
El resultado fue BP = 3.5641, con 5 grados de libertad y un p-value = 0.6137. Dado que el p-value es muy superior al nivel de significancia convencional (0.05), no hay evidencia suficiente para rechazar la hipótesis nula de homocedasticidad. En consecuencia, No hay evidencia de falta de homocedasticidad

### Distribuccion normal de los residuos

Test de Jarque Bera

-   Hipótesis del test: Hipotesis nula (H0):Los datos siguen una distribucción normal
...
-   p-value \< 0.05, los residuos no son ruido blanco, lo que indica que el modelo no captura toda la estructura de los datos y existe autocorrelación

-   p-value \> 0.05, los residuos son ruido blanco, lo que indica que el modelo captura bien la estructura de los datos y no hay autocorrelación significativa.
...
• Hipótesis nula (H₀): Los residuos no se comportan como ruido blanco (no son estacionarios). • Hipótesis alternativa (H₁): Los residuos se comportan como ruido blanco (son estacionarios).
...
Para evaluar la normalidad de los residuos se aplica la prueba de Jarque Bera . Hipótesis nula (H₀): Los residuos siguen una distribución normal. Hipótesis alternativa (H₁): Los residuos no siguen una distribución normal.
